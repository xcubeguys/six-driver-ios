...
class ADRegisterVC: UIViewController,UITextFieldDelegate {
    ...
    @IBOutlet weak var linkLineLabel: UILabel!
    ...
    @IBOutlet weak var textFieldNickName: HoshiTextField!
    @IBOutlet weak var labelCountry: HoshiTextField!

    @IBOutlet weak var termsAgreeCheck: UIImageView!
...
//    @IBOutlet var labelCountry: UILabel!
    ...
    func errorField(){
        ...
        let nicktNameTrim = textFieldNickName.text?.trimmingCharacters(in: .whitespaces)
        ...
        if(nicktNameTrim! == "" || (nicktNameTrim?.characters.count)! < 3){
        ...
        if(mobileTrim! == "" || (mobileTrim?.characters.count)! < 8){
           print(mobileTrim?.characters.count)
        ...
        if(termsAgreeCheck.image == UIImage(named: "checkbox.png")){
    ...
    func textFieldShouldEndEditing(_ textField: UITextField) -> Bool {
        ...
        if (termsAgreeCheck.image == UIImage(named: "checking-square.png")){
    ...
    override func viewDidLoad() {
        ...
        self.appDelegate.loginType = "SIX DRIVER"
        // Do any additional setup after loading the view.
        self.termsAgreeCheck.image = UIImage(named : "checkbox.png")
     ...
     /*   NotificationCenter.defaultCenter.addObserver(self, selector: #selector(ADSignInVC.keyboardWillShow(_:)), name:UIKeyboardWillShowNotification, object: nil)
        ...
        view.addGestureRecognizer(tapGesture)
         ...
         viewScroll.isScrollEnabled = true
         viewScroll.contentSize.height = 600
         
            self.checkBoxLabel.frame = CGRect(x:17, y:436, width:22, height:22)
            self.termsConditionLabel.frame = CGRect(x:160, y:435, width:135, height:21)
            self.linkLineLabel.frame = CGRect(x:163, y:453, width:130, height:1)
            self.termsAgreeCheck.frame = CGRect(x:17, y:436, width:18, height:18)
            self.agreeLabel.frame = CGRect(x:43, y:435, width:220, height:21)
...
//        fnameLabel.frame.size = self.textFieldFirstName.bounds.size
        let tap = UITapGestureRecognizer(target: self, action: #selector(ADRegisterVC.tapFunction))
        termsConditionLabel.isUserInteractionEnabled = true
        termsConditionLabel.addGestureRecognizer(tap)        // Do any additional setup after loading the view.
        
        timer = Timer.scheduledTimer(timeInterval: 1200, target: self, selector: #selector(ADRegisterVC.terminateApp), userInfo: nil, repeats: true)
        let resetTimer = UITapGestureRecognizer(target: self, action: #selector(ADRegisterVC.resetTimer));
    ...
    override func viewDidLayoutSubviews() {
        ...
        viewScroll.contentSize = CGSize(width: 320, height: 600)
    }
    ...
    */
    ...
    @IBAction func agreeButton(_ sender: Any) {
        if termsAgreeCheck.image == UIImage(named: "checkbox.png"){
            ...
            self.termsAgreeCheck.image = UIImage(named : "checking-square.png")
            self.errorAgreeLabel.isHidden = true
            
        }
        else if termsAgreeCheck.image == UIImage(named : "checking-square.png"){
    ...
    */
    ...
    func textFieldShouldBeginEditing(_ textField: UITextField) -> Bool {
        ...
        if textField == textFieldFirstName{
            ...
            textFieldNickName.borderActiveColor = UIColor.black
            textFieldNickName.borderInactiveColor = UIColor.black
            textFieldNickName.placeholderColor = UIColor.black
 ...
 /*  func delayWithSeconds(_ seconds: Double, completion: @escaping () -> ()) {
    ...
    */
    ...
    func continueBtnAction(){
        ...
        if(firstNameTrim == "" || lastNameTrim == "" || nicktNameTrim == "" || emailTrim == "" || mobileTrim == "" || confirmTrim == "" || cityTrim == "" || passwordTrim == ""){
            self.invalidData()
        ...
        else if termsAgreeCheck.image == UIImage(named : "checkbox.png"){
            self.errorAgreeLabel.isHidden = false
        }
        else{
          ...
          /*  let str = "\(passwordTrim)"
            ...
            self.appDelegate.nickname = nicktNameTrim
