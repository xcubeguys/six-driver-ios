...
//        self.maxsize=self.maxsize-1
    ...
    func msgAction(msgNum : NSMutableArray){
        ...
        if (MFMessageComposeViewController.canSendText()) {
            ...
            let space = "\(self.appDelegate.referalcode!)"
            print(space)
    ...
    if result == .cancelled {
        ...
        warning.configureDropShadow()
        let iconText = "" //"😶"
        warning.configureContent(title: "", body: "Your referal codes sending cancelled", iconText: iconText)
        warning.button?.isHidden = true
        var warningConfig = SwiftMessages.defaultConfig
        warningConfig.presentationContext = .window(windowLevel: UIWindowLevelStatusBar)
        SwiftMessages.show(config: warningConfig, view: warning)
                ...
                for record:ABRecord in people {
                    ...
                    if((ABRecordCopyCompositeName(contactPerson)) != nil)
                    {
                        ...
                        if (ABMultiValueGetCount(numbers) > 0) {
                            ...
                            {
                                let tmpimg:UIImage = UIImage(named:"UserPic_old.png")!
                                let tmdata:Data = UIImageJPEGRepresentation(tmpimg, 1.0)!
                                self.contactimg.add(tmdata)
